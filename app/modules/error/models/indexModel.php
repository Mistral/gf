...
class indexModel extends gfModel {
    ...
    public function genLink() {
       $string = "abcdefghijklmnoprstuwxyzABCDEFGHIJKLMNOPRSTUWXYZ1234567890"; // mo¿na dodaæ jeszcze du¿e litery - DONE
    ...
    public function add($link, $skrot) {
	 ...
	 public function getLink($sShort) {
		$oStmt = gfFW::MySQL()->prepare('SELECT `link` FROM `'.gfFW::MySQL()->getPrefix().'links` WHERE `skrot` = :1');
	   	$oStmt -> addParam(':1', $sShort, gfMySQL_PARAM_STR);
	   	$aFetch = $oStmt -> execute() -> fetchArray();
        if(isset($aFetch['link'])) {
            return $aFetch['link'];
        }
        return false;
    }
